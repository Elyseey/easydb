...
export const CreateDatabaseModal: React.FC<CreateDatabaseModalProps> = ({
  open, connectionId, connectionName, dbType = 'mysql', onClose, onSuccess,
}) => {
  ...
  useEffect(() => {
    ...
    if (isDameng) {
      setCharsets([])
      setCollations([])
      form.setFieldsValue({ charset: undefined, collation: undefined })
      return
    }
  ...
  }, [open, connectionId, form, isDameng])
  ...
  const updatePreview = useCallback(() => {
    ...
    if (name) {
      setSqlPreview(
        isDameng
          ? `CREATE SCHEMA "${String(name).replace(/"/g, '""')}";`
          : `CREATE DATABASE \`${name}\` CHARACTER SET ${charset || 'utf8mb4'} COLLATE ${collation || 'utf8mb4_general_ci'};`,
      )
    } else {
      setSqlPreview('')
    }
  ...
  const handleOk = async () => {
    try {
      ...
      toast.success(`${isDameng ? 'Schema' : '数据库'}「${values.name}」创建成功`)
      form.resetFields()
      onSuccess()
      onClose()
    } catch (e: unknown) {
      if (e && typeof e === 'object' && 'errorFields' in e) return // 表单校验失败
      handleApiError(e, isDameng ? '创建 Schema 失败' : '创建数据库失败')
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <Modal
      title={`${isDameng ? '新建 Schema' : '新建数据库'} — ${connectionName}`}
      open={open}
      onOk={handleOk}
    ...
    >
      ...
      >
        <Form.Item
          name="name"
          label={isDameng ? 'Schema 名称' : '数据库名称'}
          rules={[
            { required: true, message: isDameng ? '请输入 Schema 名称' : '请输入数据库名称' },
            { pattern: /^[a-zA-Z0-9_\-\u4e00-\u9fa5]+$/, message: '名称只能包含字母、数字、下划线、连字符和中文' },
          ]}
        >
          <Input placeholder={isDameng ? '请输入 Schema 名称' : '请输入数据库名称'} autoFocus />
        </Form.Item>
