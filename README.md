...
> ## 本项目已停止维护
>
> EasyDB 与 [dbx](https://github.com/t8y2/dbx/) 在产品定位与能力范围上高度重合，而 **dbx 功能更完整、维护更活跃**。为避免重复造轮子，本仓库即日起 **停止更新并归档**。
>
> 如需继续使用，或跟进同类数据库管理能力，请移步：
>
> **https://github.com/t8y2/dbx/**
>
> 感谢每一位关注与试用过 EasyDB 的朋友。历史代码与文档仍保留在本仓库，仅供参考；后续不再接受新功能与 Issue 跟进。
...
- 存储过程/函数/触发器 DDL 查看
- 表结构设计器、DDL 查看
- TDengine 时序对象浏览与创建：超级表、普通表、子表分页/搜索、Tags、DDL、只读数据预览，以及服务端 DDL 预览确认
- TDengine 时序查询：默认最近 1 小时，支持相对/绝对/不限时间、WHERE 与排序、固定边界继续加载，以及当前已加载结果的 CSV/JSON 导出
...
| **v1.4.0** | 🚧 开发中 | **达梦 + TDengine 数据库兼容** — 多数据库 SPI、方言能力矩阵、TDengine 时序对象浏览与可视化创建工作台 |
