...
| 工作台 | 超级表、普通表、子表分页/搜索、字段、Tag 定义/值、原生 DDL、只读预览、三类对象的专属可视化创建器，以及带时间范围的时序查询与当前结果导出 |
...
TDengine 表、子表和超级表的数据页使用专属时序查询契约，不复用关系型预览 SQL 拼接。首次打开默认查询最近 1 小时，预设支持最近 15 分钟、1 小时、6 小时、24 小时、7 天、自定义绝对范围和不限时间。

* 时间输入按操作系统本机时区展示，发送到内核时转换为带明确 UTC offset 的 RFC3339/ISO-8601 文本；自定义边界最多保留 9 位小数，避免 `us/ns` 数据库的边界文本被前端截断。
* 时间范围与自定义 WHERE 独立保存；内核用括号包裹后以 `AND` 组合。默认按 `_rowts DESC`，显式列排序覆盖默认排序，清除排序后恢复时间倒序。
* 每页最多返回 1000 行。内核实际多取 1 行生成 `hasMore`；筛选和排序状态仍可继续加载。
* 相对范围第一次查询时固定为绝对 start/end。继续加载沿用同一边界；用户点击刷新时才按新的当前时间重新锚定。
* “不限时间”会提示可能进行大范围扫描，但仍按页加载，不一次性读取全表。
* 导出只支持 CSV/JSON，来源严格是表格当前已加载行；当 `hasMore=true` 时确认框会提示结果尚未全部加载。TDengine 不提供 SQL INSERT 导出或后台全量导出。
...
* 数据预览仍固定只读，不提供行编辑、通用关系型表设计器、重命名、清空或删除入口。TDengine 创建和时序查询分别通过专属 adapter 开放；通用表创建路由仍返回 `UNSUPPORTED_DB_FEATURE`。
